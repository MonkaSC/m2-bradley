...
function EFFECT:Muzzle( pos, dir, vel )
	if not self.emitter then return end
      ...
      if particle then
        particle:SetVelocity( dir * i * 500 + vel )
   ...
   for i = 0,3 do
		local particle = self.emitter:Add( "particle/smokesprites_0005", pos )
  	...
  	for i = 0,3 do
		...
		if particle then
			particle:SetVelocity( dir:Angle():Right() * math.Rand(300, 1300) + vel )
			particle:SetDieTime( math.Rand(0.5,2) )
			particle:SetAirResistance( math.Rand(300,600) )
			particle:SetStartAlpha( math.Rand(25,50) )
			particle:SetStartSize( math.Rand(15,20) )
